...
import { Layout, Typography, Card, Menu } from 'antd';
import {
  CalendarOutlined,
  FileTextOutlined,
  CustomerServiceOutlined,
} from '@ant-design/icons';
import HeaderSaibaMais from './components/HeaderAbout';
...
const AboutPage = () => {
  ...
  const [selectedKey, setSelectedKey] = useState('evento');
  ...
  return (
    <Layout style={{ minHeight: '100vh' }}>
      <HeaderSaibaMais onBack={() => navigate("/home")} />
    <Layout>
        <Sider width={256} style={{ background: '#fff', borderRight: '1px solid #f0f0f0' }}>
          <Menu
            mode="inline"
        ...
        <Content style={{ minHeight: '100%', background: '#fff' }}>
  {Array.isArray(contentMap[selectedKey]) ? (
    contentMap[selectedKey].map((item, index) => (
      <Card
        key={index}
        bordered={false}
        style={{ width: '100%', marginBottom: '24px', boxShadow: '0px 2px 8px rgba(0, 0, 0, 0.1)' }}
      >
        <div style={{ backgroundColor: '#e6f7ff', padding: '16px' }}>
    ...
    <Card bordered={false} style={{ width: '100%', boxShadow: '0px 2px 8px rgba(0, 0, 0, 0.1)' }}>
      <div style={{ backgroundColor: '#e6f7ff', padding: '16px' }}>
        <Title level={5} style={{ color: '#1890ff', margin: 0 }}>
          {contentMap[selectedKey].title}
        </Title>
      </div>
      <div style={{ padding: '24px' }}>
        <Paragraph style={{ whiteSpace: 'pre-line' }}>
          {contentMap[selectedKey].content}
        </Paragraph>
      </div>
    </Card>
  )}
